...
public class GroveryManagmentDatabase extends SQLiteOpenHelper {
    ...
    private static final String DATABASE_NAME = "GroveryDB";
    ...
    /** Context where the database construct is called*/
     ...
     * The database is opened only in read mode.
     * @param nameShoppingList String value that specify the shopping list name to get its products.
     * @return A Cursor object that contains the value of product table columns.
     */
    public Cursor getAllProductsOfAList(String nameShoppingList) {
        this.db = getReadableDatabase();

        int id = getIdShoppingListByName(nameShoppingList);
        ...
        String query = "SELECT * FROM " + PRODUCT_TABLE_NAME
                ...
                + " WHERE " + PRODUCT_TABLE_NAME + "." + ID_SHOPPING_LIST_FK_COLUMN + "=" + id + "";
        Cursor cur = db.rawQuery(query, null);
     ...
     * @param nameShoppingList String value that represents the name of the shopping list which the product has been inserted into.
     */
    public void insertProduct(String nameProduct, int amountProduct, String nameShoppingList) {
        this.db = getWritableDatabase();
        ...
        try {
            ...
            cv.put(ID_SHOPPING_LIST_FK_COLUMN, id);
    ...
    /**
     * Private method use to get the shopping list ID from the name of its.
     * @param nameShoppingList String value that represents the name of the shopping list. Used to get the ID.
     * @return Integer value that represents the shopping list ID.
     */
    private int getIdShoppingListByName(String nameShoppingList) {
        String query = "SELECT " + ID_SHOPPING_LIST_COLUMN + " FROM " + SHOPPING_LIST_TABLE_NAME
                + " WHERE " + NAME_SHOPPING_LIST_COLUMN + "='" + nameShoppingList + "'";
     ...
     * The database is opened in read/write mode.
     * @param idRecipe Integer value that represents the ID of the shared recipe.
     ...
     */
    public void insertSharedRecipe(int idRecipe, String nameRecipe, String descriptionRecipe) {
        this.db = getWritableDatabase();

        this.db.beginTransaction();
        try {
            ContentValues cv = new ContentValues();
            cv.put(ID_RECIPE_COLUMN, idRecipe);
            cv.put(NAME_RECIPE_COLUMN, nameRecipe);
            cv.put(DESCRIPTION_RECIPE_COLUMN, descriptionRecipe);
    ...
    /**
     * Method use to get a Recipe object by its ID.
     * @param idRecipe Integer value that represents the ID of the recipe to get.
    ...
    public Recipe getRecipeById(int idRecipe) {
        ...
        String query = "SELECT * " + "FROM " + RECIPE_TABLE_NAME
                + " WHERE " + ID_RECIPE_COLUMN + "=" + idRecipe;

        Cursor cur = this.db.rawQuery(query, null);
        cur.moveToFirst();

        imageInByte = cur.getBlob(3);

        Bitmap image;
        if (imageInByte == null) {
            image = BitmapFactory.decodeResource(this.context.getResources(), R.mipmap.ic_error_image);
        } else {
            image = BitmapFactory.decodeByteArray(imageInByte, 0, imageInByte.length);
        }

        return new Recipe(cur.getInt(0), cur.getString(1), cur.getString(2), image);
    }
     ...
     */
    public boolean isProductInsideShoppingList(String nameProduct, String nameShoppingList) {
        this.db = getReadableDatabase();
    ...
    /**
     * Method used to check if an ingredient is already present insed a specific recipe.
     ...
     * @param nameShoppingList String that represents the name of shopping list which the product belongs to.
     */
    public void setProductAsTaken(String nameProduct, String nameShoppingList) {
        this.db = getWritableDatabase();
        ...
        this.db.update(PRODUCT_TABLE_NAME, cv, NAME_PRODUCT_COLUMN + "='" + nameProduct + "' AND " + ID_SHOPPING_LIST_FK_COLUMN + "=" + id, null);
    }

    /**
     * Method used to check if a shared recipe has already been saved.
     * The database is opened only in read mode.
     * @param idRecipe Integer value that represents the ID of the recipe.
     * @return True if the shared recipe has been already saved, false otherwise.
     */
    public boolean isRecipeAlreadyPresentInDb(int idRecipe) {
        this.db = getReadableDatabase();
