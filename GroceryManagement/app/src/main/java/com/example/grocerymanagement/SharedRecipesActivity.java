...
import classRecyclerView.CustomAdapter.CustomAdapterSharedRecipe;
...
public class SharedRecipesActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        recyclerViewSharedRecipes.setItemAnimator(new CustomAnimatorSlideInOut());

        database = new GroveryManagmentDatabase(this);

        adapterSharedRecipe.setOnClickListener(recipeToSave -> {
            ExecutorService insertDataIntoDatabaseService = Executors.newSingleThreadExecutor();
            insertDataIntoDatabaseService.submit(() -> {
                boolean isAlreadyPresent = database.isRecipeAlreadyPresentInDb(recipeToSave.getNameRecipe());

                if (!isAlreadyPresent) {
                    database.insertSharedRecipe(recipeToSave.getNameRecipe(), recipeToSave.getDescriptionRecipe());
            ...
            insertDataIntoDatabaseService.shutdown();
        });
    }
